...
SELECT `Dist Name`,
       (AVG(CASE WHEN `Year` = (SELECT MAX(`Year`) FROM agri_data) 
                 THEN `WHEAT YIELD (Kg per ha)` END) -
        AVG(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM agri_data) 
...
SELECT `State Name`,
       ( (SUM(CASE WHEN `Year` = (SELECT MAX(`Year`) FROM agri_data)
                   THEN `OILSEEDS PRODUCTION (1000 tons)` END) -
          SUM(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM agri_data)
                   THEN `OILSEEDS PRODUCTION (1000 tons)` END)
        )
        / NULLIF(SUM(CASE WHEN `Year` = (SELECT MAX(`Year`)-4 FROM agri_data)
...
WHERE `State Name` IN ('Uttar Pradesh', 'Punjab', 'Madhya Pradesh', 'Bihar', 'West Bengal') -- replace with your top 5
  AND `Year` >= (SELECT MAX(`Year`) - 9 FROM agri_data)  -- last 10 years
GROUP BY `Year`, `State Name`
ORDER BY `Year`, `State Name`;